...
| `questioning` | You have completed initial inspection and found **implicit decisions, ambiguities, or risks** that require orchestrator input before safe implementation can continue. This is part of the orchestrator's 2-phase workflow: coder inspects and surfaces questions → orchestrator resolves → coder implements. Do NOT modify files in this state. | You can resolve the decision yourself within scope. You have not inspected the code yet. The orchestrator has already given a direct instruction with no ambiguity. |
